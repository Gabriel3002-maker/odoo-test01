    ...
    def _compute_name(self):
        for work_entry in self:
            if not work_entry.employee_id:
                ...
                work_entry.display_name = _('Undefined')
            ...
            #     work_entry.display_name = "%s: %s" % (work_entry.work_entry_type_id.name or _('Undefined Type'), work_entry.employee_id.name)
            else:
                work_entry.name = "%s" % (work_entry.employee_id.name)
                work_entry.display_name = "%s" % (work_entry.employee_id.name)
